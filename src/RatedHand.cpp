...
std::vector<Board> RatedHand::getBoards() const {
  ...
  if (remainingCardsNum < 5) {
    ...
    for_each_combination(remainingCards.begin(),
                         remainingCards.begin() + remainingCardsNum,
                         remainingCards.end(),
...
double RatedHand::getSimulatedScore() const {
  ...
  for (const auto &board : boards) {
    ...
    std::sort(holeCards.begin(), holeCards.end());
    std::vector<Card52> intersection;
    std::set_intersection(handCards.begin(), handCards.end(), holeCards.begin(),
                          holeCards.end(), std::back_inserter(intersection));
